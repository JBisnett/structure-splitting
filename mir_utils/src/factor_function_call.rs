...
pub struct FunctionCallFactorer<'a, 'tcx: 'a> {
  tcx: TyCtxt<'a, 'tcx, 'tcx>,
  mir: &'a mut mir::Mir<'tcx>,
}

impl<'a, 'tcx: 'a> FunctionCallFactorer<'a, 'tcx> {
  pub fn new(tcx: TyCtxt<'a, 'tcx, 'tcx>, mir: &'a mut mir::Mir<'tcx>) -> Self {
    Self {
      tcx: tcx,
      mir: mir,
    }
        ...
        if let &mir::TerminatorKind::Call { ref func,
          ...
          for arg in args {
            if let op @ mir::Operand::Consume(mir::Lvalue::Projection(..)) =
              ...
              let assign_kind =
                mir::StatementKind::Assign(new_lvalue.clone(),
                                           mir::Rvalue::Use(op));
          ...
          true_mir.basic_blocks_mut()[block].terminator =
            Some(mir::Terminator {
              source_info: source_info,
